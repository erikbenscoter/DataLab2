...
def saveFile(threadNumber, client):
	...
	try:
		...
		try:
			...
			correctedString = ""
			correctedString = client.recv(1024)
			print correctedString
		except Exception,e:
			print e
		arrayHoldingPrediction = cl.predict(correctedString)
		if arrayHoldingPrediction[0] == 0:
			client.send(str("YOU ARE HEALTHY\n"))
		else:
			client.send(str("YOU ARE SICK\n"))
